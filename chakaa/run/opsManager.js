...
export async function scanHome(ns) {
  ...
  for (let i = 0; i < info.ps.length; i++) {
    let proc = info.ps[i]
    if(scrs.includes(proc.filename)){ 
    //if(proc.filename == G_NAME || proc.filename == H_NAME || proc.filename == W_NAME){
...
export async function mapNetwork(ns) {
  ...
  const scanHost = async (ns,host,depth) => {
    ...
    if(!info.root){
      ...
      info.max_threads = Math.floor(info.ram/OP_RAM);
      info.threads = Math.floor((info.ram - info.ram_used)/OP_RAM);
      swarm_size = swarm_size + info.max_threads;
    }
...
export async function installSPU(ns,info) {
  for (const scr of scrs) {
    if(ns.fileExists(scr,info.host))
      ns.rm(scr,info.host)
      //return
    await ns.scp(scr, info.host)
  }
...
export function runOPS(ns, host, threads, action, target, time){
  ...
  if(action=="hack"){
    ns.exec(H_NAME, host, threads, action, target, ns.getTimeSinceLastAug()/1000 + time)
  }else if(action=="grow"){
    ns.exec(G_NAME, host, threads, action, target, ns.getTimeSinceLastAug()/1000 + time)
  }else if(action=="weaken"){
    ns.exec(W_NAME, host, threads, action, target, ns.getTimeSinceLastAug()/1000 + time)
  }
}